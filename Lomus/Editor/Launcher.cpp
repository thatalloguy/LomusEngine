    ...
    if (ImGui::BeginTable("Projects", 4)) {
        ...
        for (auto& project : managedProjects) {
            ...
            if (ImGui::IsItemHovered()) {
                ...
                ImGui::SetTooltip("%s", std::string(project.name + " - Version: " + version +"\n \n(Double click to open)").c_str());
                ImGui::SetWindowFontScale(1.0f);
            }

            if (ImGui::IsItemHovered() && ImGui::IsMouseDoubleClicked(0)) {
                spdlog::info("[Launcher] Double click");
            }
            ...
            ImGui::Text(version.c_str()); ImGui::SameLine(); ImGui::Text("*");
...
void Launcher::loadManagedProjects(YAML::Node &node) {
    ...
    if (node["ManagedProjects"]) {
        ...
        for (const auto& project : managedProjectsFromConfig) {
            managedProjects.push_back(ProjectData{project["Name"].as<std::string>(), project["Path"].as<std::string>()});
